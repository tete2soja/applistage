...
class Controller_Monkey extends Controller_Template{
		...
		if (Input::method() == 'POST')
		{
			...
			if ($val->run())
			{
				$monkey = Model_Monkey::forge(array(
					...
					'code_postal' => Input::post('code_postal'),
	...
	public function action_edit($id = null)
	{
		...
		if ($val->run())
		{
			...
			$monkey->code_postal = Input::post('code_postal');
			$monkey->pays = Input::post('pays');
		...
		else
		{
			if (Input::method() == 'POST')
			{
				...
				$monkey->code_postal = $val->validated('code_postal');
				$monkey->pays = $val->validated('pays');
